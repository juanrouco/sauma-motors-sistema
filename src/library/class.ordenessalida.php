...
require_once('class.ordensalida.php');
require_once('class.filter.php');
require_once('class.page.php');

class OrdenesSalida extends DBAccess implements IFilterable
{
	public function ParseFilter(array $filter)
	{
		...
		if ((isset($filter['IdUnidad'])) && ($filter['IdUnidad'] != ''))
			$sql.= " AND os.IdUnidad = " . DB::Number($filter['IdUnidad']);
	...
	{
		$sql = "SELECT os.*";
		$sql.= " FROM TB_OrdenesSalida os";
		$sql.= " LEFT JOIN TB_Minutas m ON os.IdUnidad = m.IdUnidad";
	...
	public function GetByMinuta(Minuta $oMinuta)
	{
		$sql = "SELECT *";
		$sql.= " FROM TB_OrdenesSalida";
		$sql.= " WHERE IdUnidad = " . DB::Number($oMinuta->IdUnidad);	
	...
	public function GetByIdMinuta($IdMinuta)
	{
		$sql = "SELECT *";
		$sql.= " FROM TB_OrdenesSalida";
		$sql.= " WHERE IdUnidad = " . DB::Number($IdMinuta);	
	...
	public function Create(OrdenSalida $oOrdenSalida)
	{
		$arr = array
		(
			'IdUnidad' 						=> DB::Number($oOrdenSalida->IdUnidad),
		...
		if (!$this->Insert('TB_OrdenesSalida', $arr))
			return false;
	...
	public function Delete($IdOrden)
	{
		...
		if (!DBAccess::Delete('TB_OrdenesSalida', $where))
		{
