...
$yAlt = -0.5;
$oPdf->Text(17, 3.8 + $yAlt, CambiarFecha($oFacturaUsado->Fecha));
...
if ($oMinuta->Condominio)
{
	...
	exit();
	$oPdf->Text(4, 5.8 + $yAlt, $oCliente->RazonSocial . ' Y ' . $oClienteCondominio->RazonSocial);
}
else
{
	if ($oFacturaUsado->OtrosTitulares != '')
	{
		$oPdf->Text(4, 5.8 + $yAlt, $oCliente->RazonSocial . ' Y ' . $oFacturaUsado->OtrosTitulares);
	}
	else
	{
		$oPdf->Text(4, 5.8 + $yAlt, $oCliente->RazonSocial);
	}
}
$oPdf->Text(4, 6.6 + $yAlt, $oCliente->GetDomicilio());
$oPdf->Text(4, 6.95 + $yAlt, $oCliente->TelefonoCodigoArea . ' ' .$oCliente->Telefono);
$CodigoPostal = $oLocalidad->CodigoPostal;
if ($oCliente->DomicilioCodigoPostal)
	$CodigoPostal = $oCliente->DomicilioCodigoPostal;
$oPdf->Text(12, 6.6 + $yAlt, $oLocalidad->Nombre . ' - C.P.: ' . $CodigoPostal);
$oPdf->Text(12, 6.95 + $yAlt, $oProvincia->Nombre);
$oPdf->Text(4, 7.3 + $yAlt, $oTipoIva->Nombre);
if ($oCliente->ClaveFiscalNumero)
	$oPdf->Text(11, 7.3 + $yAlt, ClaveFiscalTipos::GetById($oCliente->ClaveFiscalTipo) . ': ' . $oCliente->ClaveFiscalNumero);
else
{
	$oTipoDocumento = $oTiposDocumento->GetById($oCliente->DocumentoTipo);
	
	$oPdf->Text(11, 7.3 + $yAlt, $oTipoDocumento->Codigo . ': ' . $oCliente->DocumentoNumero);
}
...
$oPdf->Text(3, 22 + $yAlt, 'ORIENTACION AL CONSUMIDOR PROVINCIA DE BUENOS AIRES');	
$oPdf->Text(3, 22.4 + $yAlt, 'TELEFONO: 0800-222-9042');	
...
$yAlt = -1.5;
/* importes */
$oPdf->Text(17, 12 + $yAlt, number_format($oFacturaUsado->Total, 2));
