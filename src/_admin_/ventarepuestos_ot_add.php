	...
	{		
		...
		{	
			...
			foreach ($Ids as $Id)
			{
				...
				if ($oCompraDetalleAux = $CompraDetalles->GetAllByTareaAndArticulo($IdOrdenTrabajoTarea, $oArticulo->IdArticulo))
				{
					...
					$oCompraDetalle->PrecioCompra 		= $oCompraDetalleAux->PrecioCompra;
				}
				else
				{
			...
			if (TipoVenta::OrdenReparacion == $IdTipoOperacion)
			{
...
<script type="text/javascript">
	...
	function FilterOrdenTrabajo(IdOrdenTrabajo, Dominio)
	{
		...
		$j('#lblTallerUnidad').html('<strong>OT N&deg;:</strong> ' + oOrdenTrabajo.IdOrdenTrabajo + ' - <strong>Modelo:</strong> ' + oTallerUnidad.Modelo);
		$j('#DatoVenta').html('<strong>OT N&deg;:</strong> ' + oOrdenTrabajo.IdOrdenTrabajo + ' - <strong>Modelo:</strong> ' + oTallerUnidad.Modelo);
	...
	function validar(tab, busqueda)
	{
		switch(tab) {
			case 0:
				if ($j('#IdTipoOperacion').val() != '0' && ($j('#IdOrdenTrabajo').val() == '' || $j('#IdOrdenTrabajo').val() == '0' || $j('#IdOrdenTrabajoTarea:checked').length == 0)) {
					$j('.error_2').show();
					return false;
				} else {
					$j('.error_2').hide();
		...
		if (IdArticulo != '' && IdArticulo != null && IdArticulo != undefined) {
			if ($j('#id_' + IdArticulo).length == 0) {
				...
				$j('#cantidad_' + oArticulo.IdArticulo).change(function(e) {
					...
					var cantidad = parseFloat($j(this).val());
					var precio = parseFloat($j('#PrecioUnitario_' + idAux).val());
					var unidadventa = parseInt($j('#unidadventa_' + idAux).val());
					var cantidad_anterior = parseFloat($j('#cantidad_anterior_' + idAux).val());
					if (cantidad && cantidad % unidadventa > 1)
					{
				...
				$j('#cantidad_' + IdArticulo).val(parseFloat($j('#cantidad_' + IdArticulo).val()) + parseInt(oArticulo.UnidadVenta));
				var cantidad = parseFloat($j('#cantidad_' + IdArticulo).val());
    	...
    	<td>
			<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
      			<tr>
        			...
        			<td height="40"><span class="tituloPagina">ASIGNAR REPUESTOS A OT</span></td>
      			</tr>
    		</table>
		</td>
  	</tr>
    	...
    	<td>
			<div id="tabs" align="center">
				<form name="frmData" id="frmData" method="post" enctype="multipart/form-data">
					...
					<input type="hidden" name="IdTipoOperacion" id="IdTipoOperacion" value="<?= TipoVenta::OrdenReparacion ?>" />
					<ul>
						<li><a href="#cliente-unidad">ORDEN DE TRABAJO</a></li>
					...
					<div id="repuestos">
						...
						<table width="100%"  border="0" align="center" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
							<tr>
								<td height="30">
									<div align="right">
									<input type="button" name="btnCancelar" class="botonBasico" id="btnCancelar_3" onclick="javascript: window.location.href = 'articulos.php<?=$strParams?>';" value="Cancelar" />
					...
					<div id="facturas">
							...
							<tr>
								<td>
									<table width="90%" border="0" cellpadding="0" cellspacing="0" align="center">																				
										...
										/*
						<div style="clear:both; height: 20px">&nbsp;</div>
						<div class="titulo-seccion" style="text-align: left">Cup&oacute;n:</div>
						<div style="clear:both; height: 0px">&nbsp;</div>
						...
						<div style="clear:both; height: 20px">&nbsp;</div>
						<div class="titulo-seccion" style="text-align: left">Remito:</div>
						<div style="clear:both; height: 0px">&nbsp;</div>
						<table width="100%" border="0" cellpadding="5" cellspacing="0">
							<tr>
								<td class="bordeGris">
									<table width="100%" border="0" cellpadding="0" cellspacing="0">	
										<td>Nro. Remito:&nbsp;</td>
											<td>
												<input type="text" name="NumeroComprobanteRemito" id="NumeroComprobanteRemito" class="camporFormularioMediano" maxlength="8" value="<?=$oComprobante->Numero?>" autocomplete="Off" />
												<script language="javascript">
												arrParamsRemito['FilterIdTipoComprobante'] = IdTipoComprobanteRemito;
												arrParamsRemito['FilterIdEstado'] = '<?=ComprobanteEstados::Libre?>';
												SUGGESTRequest('Comprobantes', 'GetAll', 'NumeroComprobanteRemito', 'SetNumeroComprobanteRemito', 'IdComprobante', 'Numero', 'FilterNumero', arrParamsRemito);
												</script>
												<span style="color:#FF0000;">&nbsp;(*)</span>
											</td>
											<td width="68%">&nbsp;</td>
										...
										<tr class="error_5" style="display:none">											
											<td colspan="3"><li style="color:#FF0000;">Para finalizar la venta debe ingresar un remito.</li></td>
										</tr>
									</table>
								</td>
							</tr>
						...
						<div style="clear:both; height: 0px">&nbsp;</div>
						<div class="titulo-seccion" style="text-align: left">Datos Remito:</div>
						<div style="clear:both; height: 0px">&nbsp;</div>
						<table width="100%" border="0" cellpadding="5" cellspacing="0">
							<tr>
								<td class="bordeGris">
