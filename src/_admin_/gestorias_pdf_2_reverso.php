...
//$oPdf = new FPDF('P', 'cm', array(20.6,30.6));
...
for ($i=0; $i<$oTipoFormulario->CantidadCopias; $i++)
{
	...
	$oPdf->Text($OffsetX + 1, $OffsetY + 2.1, $oCliente->RazonSocial);
	...
	if ($ImprimeLeyenda)
	{
		...
		$oPdf->Text($OffsetX + 1, $OffsetY + 4.4, 'SI LA INSCRIPCION DEL DOMINIO DE ESTE');
		$oPdf->Text($OffsetX + 1, $OffsetY + 4.7, 'AUTOMOTOR SE PRODUJERA EN EL A' . html_entity_decode('&Ntilde;') . 'O ANTERIOR');
		$oPdf->Text($OffsetX + 1, $OffsetY + 5, 'AL CONSIGNADO EN EL PRESENTE CERTIFICADO DE');
		$oPdf->Text($OffsetX + 1, $OffsetY + 5.3, 'FABRICA COMO MODELO - A' . html_entity_decode('&Ntilde;') . 'O, REGIRA A ESTOS');
		$oPdf->Text($OffsetX + 1, $OffsetY + 5.6, 'EFECTOS EL A' . html_entity_decode('&Ntilde;') . 'O DE SU INSCRIPCION TAL COMO LO');
		$oPdf->Text($OffsetX + 1, $OffsetY + 5.9, 'ESTABLECE LA RESOLUCION EX SIM 416/82.');
	}
	
	$oPdf->SetFont('Arial', '', 8);
	
	//$oPdf->Text($OffsetX + 2.2, $OffsetY + 7.4, CambiarFecha($oFacturaUnidad->Fecha));
	
	/* Titular */
	/*$oPdf->Text($OffsetX + 2, $OffsetY + 11.9, $oCliente->RazonSocial);
	if ($oCliente->DocumentoTipo == TipoDocumento::DNI)
		$oPdf->Text($OffsetX + 5, $OffsetY + 12.8, 'X');
	if ($oCliente->DocumentoTipo == TipoDocumento::LE)
		$oPdf->Text($OffsetX + 5.7, $OffsetY + 12.8, 'X');
	if ($oCliente->DocumentoTipo == TipoDocumento::LC)
		$oPdf->Text($OffsetX + 6.5, $OffsetY + 12.8, 'X');
	if ($oCliente->DocumentoTipo == TipoDocumento::CI)
		$oPdf->Text($OffsetX + 7.2, $OffsetY + 12.8, 'X');
	if ($oCliente->DocumentoTipo == TipoDocumento::PA)
		$oPdf->Text($OffsetX + 8, $OffsetY + 12.8, 'X');
	$oPdf->Text($OffsetX + 2.5, $OffsetY + 13.6, $oCliente->DocumentoNumero);
	$oPdf->Text($OffsetX + 5.5, $OffsetY + 13.6, $oCliente->DocumentoExpedido);*/
	
	if ($Observaciones)
	{
		$oPdf->Text($OffsetX + 1, $OffsetY + 17.4, $Observaciones);
	}
	
	/* Condominio */
	if (($oGestoria->PorcentajeTitularidad) < 100 && ($oGestoria->IdClienteCondominio != ''))
	{
		$oPdf->Text($OffsetX + 9.5, $OffsetY + 11.9, $oClienteCondominio->RazonSocial);
		if ($oClienteCondominio->DocumentoTipo == TipoDocumento::DNI)
			$oPdf->Text($OffsetX + 12.6, $OffsetY + 12.8, 'X');
		if ($oClienteCondominio->DocumentoTipo == TipoDocumento::LE)
			$oPdf->Text($OffsetX + 13.4, $OffsetY + 12.8, 'X');
		if ($oClienteCondominio->DocumentoTipo == TipoDocumento::LC)
			$oPdf->Text($OffsetX + 14.1, $OffsetY + 12.8, 'X');
		if ($oClienteCondominio->DocumentoTipo == TipoDocumento::CI)
			$oPdf->Text($OffsetX + 14.8, $OffsetY + 12.8, 'X');
		if ($oClienteCondominio->DocumentoTipo == TipoDocumento::PA)
			$oPdf->Text($OffsetX + 15.5, $OffsetY + 12.8, 'X');
		$oPdf->Text($OffsetX + 10, $OffsetY + 13.6, $oClienteCondominio->DocumentoNumero);
		$oPdf->Text($OffsetX + 13, $OffsetY + 13.6, $oClienteCondominio->DocumentoExpedido);
	}
...
//$oPdf->Output('formulario_01_nacional_reverso.pdf', 'D');
$oPdf->AutoPrint(true);
/* generamos el archivo */
$oPdf->Output();
