...
if (!Session::CheckPerm(PERM_PEDMAY_UPDATE))
	Session::NoPerm();
...
{
	header('Location: pedidosmayorista.php');
	exit;
}
...
if ($Submit)
{
	...
	if ($err == 0)
	{
		...
		if ($oPedidoMayorista = $oPedidosMayorista->Update($oPedidoMayorista))
		{
			/* enviamos el id generado */
			$strParams.= '&IdPedidoMayorista=' . $oPedidoMayorista->IdPedidoMayorista;
			
			header('Location: pedidosmayorista_seleccionar_minuta_mod.php' . $strParams);
			exit;
		}
	}
}
else
{
	...
	$FechaPedidoMayorista 	= CambiarFecha($oPedidoMayorista->FechaPedidoMayorista);
	$IdCliente 				= $oPedidoMayorista->IdCliente;
	$oCliente = $oClientes->GetById($oPedidoMayorista->IdCliente);
	$Cliente = $oCliente->RazonSocial;
	$Observaciones 			= $oPedidoMayorista->Observaciones;
}
...
<table width="90%"  border="0" align="center" cellpadding="0" cellspacing="0" class="bordeGris">
	<tr>
  	...
  	<tr>
    	<td>
			<div align="center">
				<form name="frmData" id="frmData" method="post" enctype="multipart/form-data" action="pedidosmayorista_mod_paso1.php<?=$strParams?>">
