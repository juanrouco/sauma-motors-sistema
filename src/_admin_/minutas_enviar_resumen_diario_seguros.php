...
$filter	= array();
$filter['FechaFinDesde'] = date('d-m-Y', strtotime("-1 days"));
$filter['FechaFinHasta'] = date('d-m-Y', strtotime("-1 days"));		
$filter['ReportadoSeguros'] = '0';
$filter['Facturado'] = '1';

$arrOrdenesTrabajo = $oOrdenesTrabajo->GetAll($filter);

print_r($arrOrdenesTrabajo);exit;
if ($arrOrdenesTrabajo)
{
	...
	foreach ($arrOrdenesTrabajo as $oOrdenTrabajo)
	{
		...
		if ($oCliente->Email && strpos($oCliente->Email, 'NOPOSEE') === false)
		{
			...
			<tr>
				<td>Por favor, haga clic <a href="http://motopier.com.ar/control_de_calidad/encuesta_servicio_tecnico.php">aqu&iacute;</a> y complete la encuesta.</td>
			</tr>
			<tr>
				<td>&nbsp;</td>
			</tr>
