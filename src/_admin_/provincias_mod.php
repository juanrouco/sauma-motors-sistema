...
if (!Session::CheckPerm(PERM_PROV_UPDATE))
	Session::NoPerm();
...
if (!$oProvincias = $oProvincias->GetById($IdProvincia))
{
...
if ($Submit)
{
	...
	if ($Nombre == '')
		$err |= 1;
	if (($Nombre != $oProvincias->Nombre) && ($oProvincias->GetByNombre($Nombre)))
		$err |= 2;
	if ($IdPais == '')
		$err |= 4;
	...
	if ($err == 0)
	{
		$oProvincias->Nombre = $Nombre;
		$oProvincias->IdPais = $IdPais;

		$oProvincias = $oProvincias->Update($oProvincias);
...
{
	$Nombre = $oProvincias->Nombre;
	$IdPais = $oProvincias->IdPais;
}
    	...
    	<td>
			<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
      			<tr>
        			...
        			<td height="40"><span class="tituloPagina">Administraci&oacute;n de Provincias - Modificar</span></td>
      			</tr>
    		</table>
		</td>
  	</tr>
