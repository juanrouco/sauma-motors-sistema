...
{	
	...
	{	
		...
		$oFacturaPostVenta = $oOrdenTrabajoFactura->GenerarFacturaSinGuardar($oOrdenTrabajo, $oCliente, $oComprobante, $IdPlanCuota, $Descuento, $Comentarios, $Interes);
	...
	$totaldesc = ($oOrdenTrabajo->ImporteTotalCalculado() * (100-$Descuento)/100) * (( 100 + $Interes) / 100);
	...
	if ($err == 0)
	{
		...
		$oFacturaPostVenta = $oOrdenTrabajoFactura->GenerarFactura($oOrdenTrabajo, $oCliente, $oComprobante, $IdPlanCuota, $Descuento, $Comentarios, $Interes);
		...
		header("Location: ordenestrabajo_factura_afip.php?IdFactura=" . $oFacturaPostVenta->IdFacturaPostVenta);
		exit();
	}
}
else
																				...
																				<div align="left">
																					...
																					?>
																						<tr>
																							...
																							<td>
																								<div id="margen">
																									<input type="text" class="camporFormularioChico" <?= in_array($oTipoPago['IdTipoPago'], $arrIdTipoPago) ? '' : 'style="display: none"' ?> name="Importe[]" id="Importe_<?= $oTipoPago['IdTipoPago'] ?>" value="<?= $oTipoPago['IdTipoPago'] == 1 ?  $oOrdenTrabajo->ImporteTotalCalculado() : 0 ?>" />
																								</div>
																							</td>
																						</tr>
																					<?php
