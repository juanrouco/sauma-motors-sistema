...
$Reconfirmado			= intval($_REQUEST['Reconfirmado']);
$Submit					= (isset($_REQUEST['Submitted']));

$err				= 0;
$oTurno		= new Turno();
$oOrdenesTrabajo	= new Turnos();
...
if (!$oTurno = $oOrdenesTrabajo->GetById($IdTurno))
{
...
if ($Submit)
{
	...
	{		
		...
		$oTurno->Bahia				= $Bahia;
		$oTurno->Remis				= $Remis;
		$oTurno->Reconfirmado		= $Reconfirmado;
		
		$oTurno = $oOrdenesTrabajo->Update($oTurno);
...
else
{
	...
	$FechaInicio	= date_format(new DateTime($oTurno->FechaInicio), 'd-m-Y');
	$FechaFin		= date_format(new DateTime($oTurno->FechaFin), 'd-m-Y');
	$HoraInicio		= date_format(new DateTime($oTurno->FechaInicio), 'H');
	$HoraSalida		= date_format(new DateTime($oTurno->FechaFin), 'H');
	$MinutoInicio	= date_format(new DateTime($oTurno->FechaInicio), 'i');
	$MinutoSalida	= date_format(new DateTime($oTurno->FechaFin), 'i');
	$IdUsuario		= $oTurno->IdUsuarioAsignado;
	$Kilometros		= $oTurno->Kilometros;
	$Comentarios	= $oTurno->Comentarios;
	$Bahia			= $oTurno->Bahia;
	$Remis			= $oTurno->Remis;
	$Reconfirmado	= $oTurno->Reconfirmado;
    	...
    	<td>
			<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
      			<tr>
        			...
        			<td height="40"><span class="tituloPagina">Modificar Turno</span></td>
      			</tr>
    		</table>
		</td>
  	</tr>
																		...
																		<tr>
																			<td>
																				<table width="100%" cellpadding="0" cellspacing="0" border="0">
																					...
																					<tr>
																						<td valign="niddle">
																							<div align="right">Fecha Egreso: &nbsp;</div>
																						</td>
																						<td valign="top">
																					...
																					<tr>
																						<td valign="niddle">
																							<div align="right">Horario Egreso: &nbsp;</div>
																						</td>
																						<td valign="top">
																					...
																					<tr>
																						<td valign="niddle">
																							<div align="right">Reconfirmado: &nbsp;</div>
																						</td>
																						<td valign="top">
																							<input type="checkbox" name="Reconfirmado" id="Reconfirmado" value="1" <?= $Reconfirmado ? 'checked="checked"' : '' ?> />
																						</td>
																					</tr>
																					<input type="hidden" id="IdEstadoOrden" name="IdEstadoOrden" value="<?= $IdEstadoOrden ?>" />
																					<?php /*<tr>
						...
						<tr>
							<td height="30">
								<div align="center">
									...
									<input type="button" name="btnCancelar" class="botonBasico" id="btnCancelar" onclick="javascript: window.location.href = 'turnos.php<?=$strParams?>';" value="Cancelar" />
								</div>
							</td>
						</tr>
					</table>
