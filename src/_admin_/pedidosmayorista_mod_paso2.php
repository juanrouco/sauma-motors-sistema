...
if ($Submit)
{
	...
	switch ($Action)
	{
		case 'Select':
			...
			$oPedidosMayoristaDetalles->DeleteByIdPedidoMayorista($IdPedidoMayorista);
			...
			foreach ($arrIdMinuta as $IdMinuta)
			{
				...
				if ($err == 0)
				{
					if (!$oPedidoMayoristaDetalle = $oPedidosMayoristaDetalles->GetById($IdPedidoMayorista, $IdMinuta))
					{
						$oPedidoMayoristaDetalle = new PedidoMayoristaDetalle();
						$oPedidoMayoristaDetalle->IdMinuta				 = $IdMinuta;
						$oPedidoMayoristaDetalle->IdPedidoMayorista		 = $oPedidoMayorista->IdPedidoMayorista;
						
						$oPedidoMayoristaDetalle = $oPedidosMayoristaDetalles->Create($oPedidoMayoristaDetalle);
					}
				}
			}
		...
		case 'Delete':
			...
			$IdMinuta = intval($_REQUEST['Id']);
			
			$oPedidoMayoristaDetalle = $oPedidosMayoristaDetalles->GetById($IdPedidoMayorista, $IdMinuta);
			$oPedidosMayoristaDetalles->Delete($oPedidoMayoristaDetalle->IdPedidoMayorista, $oPedidoMayoristaDetalle->IdMinuta);
		...
		case 'Next':
			...
			if (count($err) == 0)
			{
				$oPedidoMayorista->IdEstado = PedidosMayoristaEstados::Aprobado;
				$oPedidosMayorista->Update($oPedidoMayorista);
                ...
                <?php foreach ($arrData as $oPedidoMayorista) { ?>
                    ...
                    <tr onMouseOver="bgColor='#f3f3f3'" onMouseOut="bgColor=''">
                        ...
                        <td width="85" height="25">
                            <div align="center"> 
                                <a href="javascript: void(0);" onclick="Delete('<?=$oPedidoMayorista->IdMinuta?>');"><img src="images/iconos/del.gif" border="0" /></a>
                            </div>
                        </td>
                    </tr>
                    <tr>
        	...
        	<td>
                <table width="90%"  border="0" align="center" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
                    <tr>
                        <td height="30">
                            <div align="center">
                            	<input type="button" name="btnSiguiente" class="botonBasico" id="btnSiguiente" value="Finalizar" onClick="javascript: Next();" />
                            </div>
                        </td>
                    </tr>
                </table>
