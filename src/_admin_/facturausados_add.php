...
if (!Session::CheckPerm(PERM_FACTUS_CREATE))
	Session::NoPerm();
...
{
	/* validaciones... */
	if (($IdMinuta == '') || (!$oMinutasUsados->GetById($IdMinuta)))
		$err |= 1;
	...
	if ($oMinutasUsados->GetById($IdMinuta))
	{
	...
	if ($err == 0)
	{
		$oFacturaUsado->IdMinuta			= $IdMinuta;
		$oFacturaUsado->IdComprobante		= $IdComprobante;
		$oFacturaUsado->NumeroComprobante	= $NumeroComprobante;
		$oFacturaUsado->Fecha				= $Fecha;
		$oFacturaUsado->Subtotal			= $Subtotal;
		$oFacturaUsado->Iva10				= $Iva10;
		$oFacturaUsado->Iva21				= $Iva21;
		$oFacturaUsado->Total				= $Total;
		$oFacturaUsado->ImpuestoInterno		= $ImpuestoInterno;
		$oFacturaUsado->OtrosTitulares		= $OtrosTitulares;
		$oFacturaUsado->Observaciones		= $Observaciones;

		if ($oFacturaUsado = $oFacturaUsados->Create($oFacturaUsado))
		{
			...
			if ($oComprobante = $oComprobantes->GetById($IdComprobante))
			{
				...
				$oComprobante->Fecha 		= $oFacturaUsado->Fecha;
				$oComprobante->Importe		= $Total;
				$oComprobante->ImporteIva21 = $Total / 1.21 * 0.21;
				
				$oComprobantes->Update($oComprobante);
			}

			/* Actualizamos el estado de la unidad */
			$oUsado->IdEstado = EstadoUnidad::Facturado;
...
{
	var Url = 'minutasusados.php?MainAction=SelectFacturacion';
...
debugger;
	if (!(oMinuta = GetMinutaUsado(IdMinuta)))
	{
		ShowSection('IdMinutaError');
		return;
	}

	if (!(oUsado = GetUsado(oMinuta.IdUsado)))
		return;

		
	if (!(oMarca = GetMarca(oUsado.IdMarca)))
		return;

	if (!(oColor = GetColor(oUsado.IdColor)))
		return;

	oMarcaMotor = GetMarca(oUsado.IdMarcaMotor);

	oMarcaChasis = GetMarca(oUsado.IdMarcaChasis);
	...
	Get('VehiculoAnio').innerHTML 					= oUsado.ModeloAnio;
	Get('VehiculoModelo').innerHTML 				= oUsado.Modelo;
	if (oUsado.NumeroMotor)
		Get('VehiculoNumeroMotor').innerHTML 			= oUsado.NumeroMotor + (oMarcaMotor == undefined ? '' : ' - ' + oMarcaMotor.Nombre);
	if (oUsado.NumeroChasisPrefijo == undefined)
		Get('VehiculoNumeroChasis').innerHTML 			= oUsado.NumeroChasis + (oMarcaChasis == undefined ? '' : ' - ' + oMarcaChasis.Nombre);
	else
		Get('VehiculoNumeroChasis').innerHTML 			= oUsado.NumeroChasisPrefijo + oUsado.NumeroChasis + (oMarcaChasis == undefined ? '' : ' - ' + oMarcaChasis.Nombre);
	Get('VehiculoColor').innerHTML					= oColor.Nombre;
	Get('VehiculoDenominacionComercial').innerHTML 	= oUsado.Modelo;
	Get('VehiculoIva').innerHTML 					= '21 %';
	...
	*/
		...
		ImpuestoInterno = parseFloat(oModelo.ImpuestoInterno);
    	...
    	<td>
			<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
      			<tr>
        			...
        			<td height="40"><span class="tituloPagina">Administraci&oacute;n de Facturas de Minutas de Usados - Agregar</span></td>
      			</tr>
    		</table>
		</td>
  	</tr>
                                        ...
                                        <td>
                                            <table border="0" align="center" cellpadding="0" cellspacing="0">
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div align="left">
                                                            <input type="text" name="NumeroComprobante" id="NumeroComprobante" class="camporFormularioMediano" maxlength="8" value="<?=$oComprobante->Numero?>" />
															<script language="javascript">
                                                ...
                                                <tr>
                                                	<td height="25">&nbsp;</td>
                                                	<td height="25"><?php if ($err & 2) { ?><li style="color:#FF0000;">Ingrese el nro. de factura</li><?php } ?><?php if ($err & 128) { ?><li style="color:#FF0000;">La factura seleccionada ya fue utilizada.</li><?php } ?><?php if ($err & 256) { ?><li style="color:#FF0000;">Por favor, vuelva a seleccionar la factura.</li><?php } ?><?php if ($err & 512) { ?><li style="color:#FF0000;">La factura seleccionada no existe.</li><?php } ?></td>
                                                </tr>
                                                <tr>
						...
						<tr>
							<td height="30">
								<div align="center">
									...
									<input type="button" name="btnCancelar" class="botonBasico" id="btnCancelar" onclick="javascript: window.location.href = 'facturausados.php<?=$strParams?>';" value="Cancelar" />
								</div>
							</td>
						</tr>
					</table>
