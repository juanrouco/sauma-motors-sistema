	...
	foreach ($arrTiposFormulario as $oTipoFormulario)
	{
		if ($_REQUEST['IdTipoFormulario_' . $oTipoFormulario->IdTipoFormulario])
		{
			...
			else
			{
				...
				elseif ($oFormulario->IdEstado != FormularioEstados::Libre)
				{
					$error = true;
	...
	switch ($Action)
	{
		...
		case 'Back':
			...
			if (($oMinuta->FinanciacionCapital != '') && ($oMinuta->FinanciacionCapital != 0))
			{
				...
				{
					header("Location: gestorias_add_paso4.php" . $strParams);
					exit;
				}
				else
				{
...
function VerificarFormario(IdTipoFormulario)
{
...
function AddCedula()
{
	...
	frmData.action = 'gestorias_add_paso5.php#Cedulas';
	frmData.submit();
	return true;
}
...
function AddSocio()
{
	...
	frmData.action = 'gestorias_add_paso5.php';
	frmData.submit();
	return true;
}
    	...
    	<td>
			<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
      			<tr>
        			...
        			<td height="40"><span class="tituloPagina">Administraci&oacute;n de Gestor&iacute;as - Agregar - Paso <?=(($oMinuta->FinanciacionCapital == '') || ($oMinuta->FinanciacionCapital == 0)) ? '3' : '5'?></span></td>
      			</tr>
    		</table>
		</td>
  	</tr>
                                                                    ...
                                                                    <td>
                                                                        <div align="left">
                                                                            ...
                                                                            <?php foreach ($arrTiposFormularioGestoria as $oTipoFormulario) 
																				{ 
																					$oFormulario = GestoriaCreate::GetFormulario($oTipoFormulario->IdTipoFormulario);
																					$oNextFormulario = $oFormularios->GetNext($oTipoFormulario->IdTipoFormulario);
																			?>
                                                                                <tr>
                                                                                    <td width="30">
                                                                                    ...
                                                                                    <td width="127">
                                                                                        ...
                                                                                        <table border="0" cellpadding="0" cellspacing="0">
                                                                                            <tr id="trFormulario_<?=$oTipoFormulario->IdTipoFormulario?>" style="display:none;">
                                                                                                ...
                                                                                                <td>
                                                                                                    <input type="text" name="Numero_<?=$oTipoFormulario->IdTipoFormulario?>" id="Numero_<?=$oTipoFormulario->IdTipoFormulario?>" value="<?=($oFormulario) ? $oFormulario->Numero : $oNextFormulario->Numero?>" class="camporFormularioChico" autocomplete="Off" />
                                                                                                    <script language="javascript">
