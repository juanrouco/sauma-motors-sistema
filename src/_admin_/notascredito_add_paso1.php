	...
	if ($err == 0)
	{
		...
		if ($oNotaCredito = $oNotasCredito->Create($oNotaCredito))
		{
			...
			if ($oComprobante = $oComprobantes->GetById($IdComprobante))
			{
				...
				$oComprobante->IdCliente = $IdCliente;
		...
		header("Location: notascredito_add_paso2.php" . $strParams . '&IdNotaCredito=' . $oNotaCredito->IdNotaCredito);
		exit();
	}
}
else
...
function FilterCliente(IdCliente, RazonSocial)
{
	...
	{		
		...
		oComprobante = GetNextFactura('<?=ComprobanteTipos::NotaCreditoA?>');
	...
	else if (oTipoIva.FacturaTipo == '<?=ComprobanteTipos::FacturaB?>')
	{
		...
		/* obtenemos la proxima factura */
		oComprobante = GetNextFactura('<?=ComprobanteTipos::NotaCreditoB?>');
                                    ...
                                    <tr>
                                        <td>
                                            <table border="0" align="center" cellpadding="0" cellspacing="0">
                                                ...
                                                <tr>
                                                    <td><div align="right">Nro. Nota de Cr&eacute;dito:</div></td>
                                                    <td>
                                                        <div align="left">
                                                ...
                                                <tr>
                                                	<td>&nbsp;</td>
                                                	<td colspan="2"><?php if ($err & 2) { ?><li style="color:#FF0000;">Ingrese el nro. de nota de cr&eacute;dito</li><?php } ?></td>
                                                </tr>
                                                <tr>
