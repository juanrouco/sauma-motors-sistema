...
if (!Session::CheckPerm(PERM_TIPD_CREATE))
	Session::NoPerm();
...
if ($Submit)
{
	...
	if ($Nombre == '')
		$err |= 2;
	elseif ($oTiposDocumento->GetByNombre($Nombre))
		$err |= 4;
	...
	if ($err == 0)
	{
		...
		$oTipoDocumento = $oTiposDocumento->Create($oTipoDocumento);
    	...
    	<td>
			<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
      			<tr>
        			...
        			<td height="40"><span class="tituloPagina">Administraci&oacute;n de Tipos de Documentos - Agregar</span></td>
      			</tr>
    		</table>
		</td>
  	</tr>
...
window.opener.FilterTipoDocumento('<?=$oTipoDocumento->IdTipoDocumento?>', '');
<?php } elseif ($Tipo == 'DocumentoConyugeTitular') { ?>
window.opener.FilterTipoDocumentoConyugeTitular('<?=$oTipoDocumento->IdTipoDocumento?>', '');
<?php } elseif ($Tipo == 'DocumentoConyugeCondominio') { ?>
window.opener.FilterTipoDocumentoConyugeCondominio('<?=$oTipoDocumento->IdTipoDocumento?>', '');
<?php } ?>
window.close();
</script>
<?php } ?>
