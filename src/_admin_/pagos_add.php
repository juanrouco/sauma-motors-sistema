...
$BancoDestino			= intval($_REQUEST['BancoDestino']);
...
if ($Submit)
{
	...
	if ($err == 0)
	{
		$oCajaDestino	= $oCajasDetalles->GetById($BancoDestino);
		$oPago->IdMinuta 			= $IdMinuta;
		$oPago->IdTipoPago			= $IdTipoPago;
		$oPago->Fecha	 			= $Fecha;
		$oPago->Importe			 	= $Importe;
		$oPago->BancoDestino		= $oCajaDestino->Nombre;
		...
		header("Location: pagos.php" . $strParams);
		exit();
	}
}
else
						...
						<tr>
							<td height="30">
								<div align="center">
									...
									<input type="button" name="btnCancelar" class="botonBasico" id="btnCancelar" onclick="javascript: window.location.href = 'pagos.php<?=$strParams?>';" value="Cancelar" />
								</div>
							</td>
						</tr>
					</table>
