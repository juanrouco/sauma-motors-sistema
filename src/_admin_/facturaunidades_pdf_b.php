...
//$oPdf->Text(4, 7.3 + $yAlt, $oTipoIva->Nombre);
	//$oPdf->Text(10.5 + 2.9 + 0.5,  4.6 + 4.6 + $yAlt,'CUIL: 20-31433812-0');
...
else
{
	...
	$oPdf->Text(10.5 + 2.9 + 0.5,  4.6 + 4.6 + $yAlt, $oTipoDocumento->Codigo . ': ' . $oCliente->DocumentoNumero);
}

/* datos del automotor */
$oPdf->Text(1.6, 8.3 + 3.5 + $yAlt, '1');
$oPdf->Text(3, 8.3 + 3.5 + $yAlt, 'MOTOCICLETA MARCA ' . $oMarcaVehiculo->Nombre . ' 0KM');
$oPdf->Text(3, 9 + 3.5 + $yAlt, 'Modelo:');
...
if ($oModelo->Iva == '10.5')
{
	$oPdf->Text(3, 13.5 + 3.5 + $yAlt, 'ALICUOTA:');
...
$yAlt = -1.5;
/* importes 
if ($oComprobante->ImpuestoInterno)
{
	$oPdf->Text(17, 12 + $yAlt, number_format($oFacturaUnidad->Total - $oComprobante->ImpuestoInterno, 2));
	$oPdf->Text(13, 14 + $yAlt, 'Impuesto Interno');
	$oPdf->Text(17, 14 + $yAlt, number_format($oComprobante->ImpuestoInterno, 2));
}
else*/
	$oPdf->Text(13.8 + 2.5, 9.3 + 3.5 + $yAlt, number_format($oFacturaUnidad->Total, 2));
$oPdf->Text(13.8 + 2.5, 19 + 8.4 + $yAlt, number_format($oFacturaUnidad->Total, 2));
//$oPdf->Text(2.5, 27 + $yAlt, $oNumber->ValorEnLetras($oFacturaUnidad->Total, "pesos") . $Delimitador);
