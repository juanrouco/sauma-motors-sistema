...
{
	header('Location: tareastrabajo.php');
	exit;
}
...
if ($Submit)
{
	...
	if ($err == 0)
	{
		$ImporteManoObra	= str_replace(",", ".", $ImporteManoObra);
		...
		$oTareaTrabajo = $oTareasTrabajo->Update($oTareaTrabajo);
...
else
{
	...
	$IdCodigoTrabajo		= $oTareaTrabajo->IdCodigoTrabajo;
	
	$oCodigoTrabajo = $oCodigosTrabajo->GetById($IdCodigoTrabajo);
	$CodigoTrabajo			= $oCodigoTrabajo->Descripcion;
	
	$ImporteManoObra		= $oTareaTrabajo->ImporteManoObra;
	$Titulo					= $oTareaTrabajo->Titulo;
	$Descripcion			= $oTareaTrabajo->Descripcion;
	$HorasEstimadas			= $oTareaTrabajo->HorasEstimadas;
	
	$AnioDesde				= $oTareaTrabajo->AnioDesde;
	$AnioHasta				= $oTareaTrabajo->AnioHasta;
	$Importe				= $oTareaTrabajo->Importe;
	$IdTipoCosto			= $oTareaTrabajo->IdTipoCosto;
	$IdModeloPV				= $oTareaTrabajo->IdModeloPV;
	
	$ImporteCalculado		= $oCostosManoObra->GetLast() * $oTareaTrabajo->HorasEstimadas;
}
...
$j(document).ready(function() {
	...
	$j('#IdTipoCosto').change(TipoCostoChange);
	<?php
	if ($IdTipoCosto)
	{
	?>
    	...
    	<td>
			<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
      			<tr>
        			...
        			<td height="40"><span class="tituloPagina">Administraci&oacute;n de tareas de taller - Modificar</span></td>
      			</tr>
    		</table>
		</td>
  	</tr>
