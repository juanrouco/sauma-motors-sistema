...
```php
// library/class.acreedores.php - typical pattern
...
`xml/xmlhelper.php` has the authentication check **commented out** (lines 25-36):
```php
/*
...
- Database credentials in `library/class.config.php` (root/root)
- AFIP certificates and keys stored in `facturaelectronica/` (`.crt`, `.key` files committed to repo)
...
- `register_globals` handling in `inc_library.php` (removed in PHP 5.4 but code still checks for it)
- Third-party libraries (PHPExcel, old PHPMailer 2.3, WSpooler, PEAR) may not be PHP 7 compatible

### No Dependency Management
- No Composer — all libraries manually copied into `library/`
...
- 528 files flat in `library/` with no subdirectories/namespaces
...
- Company data (`DatosEmpresa`) loaded on every page request via `inc_library.php`

### Unbounded File Storage
- `_recursos/` grows without limits (uploaded files, generated invoices)
- No cleanup mechanism for old files

## Fragile Areas

### AFIP Electronic Invoicing
- `facturaelectronica/` — Certificates have expiration dates, renewal is manual
...
- Stock adjustments in `library/class.articulostock*.php`
...
3. **Register globals cleanup** — `inc_library.php` unsets `$_SESSION` variables which could break session handling if `register_globals` were somehow enabled