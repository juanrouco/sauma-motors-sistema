...
- `library/barcodegen/test_1D.php` — Barcode generation test page
- `library/mail/tests/` — PHPMailer library tests (third-party, not project tests)
...
### High Priority
1. **Financial operations** — Invoice generation, payment processing, cash register operations (`library/class.comprobantes*.php`, `library/class.cajas*.php`)
2. **AFIP electronic invoicing** — Tax authority integration, certificate handling (`facturaelectronica/`)
3. **Inventory management** — Stock adjustments, transfers, minimum stock alerts (`library/class.articulostock*.php`)
4. **Data access layer** — SQL query correctness in repository classes

### Medium Priority
5. **Work order lifecycle** — Status transitions, task assignments (`library/class.ordentrabajo*.php`)
6. **Customer management** — CUIT validation, duplicate detection (`modules/clientes.php`)
7. **Permission system** — Access control enforcement (`inc_perms.php`, `Session::CheckPerm()`)
