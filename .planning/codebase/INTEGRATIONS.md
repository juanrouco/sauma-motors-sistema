...
- Electronic Invoice Service (Factura Electrónica)
  ...
  - Implementation: `src/library/class.facturaelectronica.php`
  ...
  - Configuration: `src/library/class.configuracionfactura.php`
  - Credentials:
...
- Service appointment synchronization
  ...
  - Implementation: `src/library/class.turnosagendaws.php`, `src/library/class.turnoagendaws.php`
  ...
  - Secrets stored in: `src/library/class.config.php.example` (lines 57-60)
...
- Vehicle service recommendations and technical data
  - Implementation: `src/library/class.codigosgudb.php`, `src/library/class.gudbserviceappreport.php`
  - Configuration in `src/library/class.config.php`:
    - `CodigoPais`: 'ARG'
...
**Databases:**
- MySQL 5.7
  - Connection: `src/library/class.db.php`
  ...
  - ORM/Access Pattern: Custom DBAccess base class in `src/library/class.dbaccess.php`
  - Key tables:
...
- Local filesystem only
  - Generated documents: `src/_recursos/comprobantes/` (invoices/receipts)
  - Uploaded resources: `src/_recursos/` (various file types)
  - Temporary uploads: `src/_recursos/` (user uploads)
...
- Custom session-based authentication
  - Implementation: `src/library/class.session.php`
...
- Azure AD OAuth2 flow
  ...
  - Client credentials in `src/library/class.config.php`
...
- Database migrations: SQL scripts in `src/sql/cambios-basedatos.sql`
...
- All sensitive data hardcoded in `src/library/class.config.php` (security risk)

**Config Location:**
- `src/library/class.config.php` (main configuration)
- `src/library/class.configuracionfactura.php` (AFIP invoice setup)
...
- Implementation: `src/library/WSpooler/WSpooler.php`
- Used by: `src/library/class.generadordocumentos.php` and related generators
...
- Custom CSV export via `src/library/excel_export/`
