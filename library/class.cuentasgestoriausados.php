...
require_once('class.cuentagestoriausado.php');
...
class CuentasGestoriaUsados extends DBAccess implements IFilterable
{
	...
	{
		$sql = "SELECT *";
		$sql.= " FROM TB_CuentasGestoriaUsados";
		$sql.= " WHERE 1";
	...
	{
		$sql = "SELECT fu.*";
		$sql.= " FROM TB_CuentasGestoriaUsados fu";
	...
	public function Create(CuentaGestoria $oCuentaGestoria)
	{
		...
		if (!$this->Insert('TB_CuentasGestoriaUsados', $arr))
			return false;
	...
	public function Update(CuentaGestoria $oCuentaGestoria)
	{
		...
		if (!DBAccess::Update('TB_CuentasGestoriaUsados', $arr, $where))
			return false;
	...
	public function Delete($IdCuentaGestoria)
	{
		...
		if (!DBAccess::Delete('TB_CuentasGestoriaUsados', $where))
		{
	...
	private function CrearMovimientoCuentaGestoria($oCuentaGestoria)
	{
		...
		if ($Crear && $oCuentaGestoria->ComisionGestor && $oCuentaGestoria->ComisionGestor > 0)
		{
			$oCajaMovimiento->IdTipoMovimiento = TiposMovimientosCaja::CuentaCorrienteUsado;
			$oCajaMovimiento->Fecha = date('Y-m-d H:i:s');
			$oCajaMovimiento->IdEntidad = $oCuentaGestoria->IdCuentaGestoria;
			$oCajaMovimiento->Total = $oCuentaGestoria->ComisionGestor * -1;
		...
		if ($Crear)
		{
			...
			$oCajaMovimiento->Fecha = date('Y-m-d H:i:s');
			$oCajaMovimiento->IdEntidad = $oCuentaGestoria->IdCuentaGestoria;
			$oCajaMovimiento->Total = ($oCuentaGestoria->TotalCalculado - $oCuentaGestoria->ComisionGestor) * -1;
			$oCajasMovimientos->Create($oCajaMovimiento);
	...
	private function CrearMovimientoCuentaGestoriaRendicion($oCuentaGestoria)
	{
		...
		if ($Crear)
		{
			$oCajaMovimiento->IdTipoMovimiento = TiposMovimientosCaja::RendicionUsado;
			$oCajaMovimiento->Fecha = date('Y-m-d H:i:s');
			$oCajaMovimiento->IdEntidad = $oCuentaGestoria->IdCuentaGestoria;
	...
	private function EliminarMovimientoCuentaGestoria($IdCuentaGestoria)
	{
		...
		$IdCajaDetalle = $oCajaDefault->IdCajaAdministracion;
		if ($oCajaMovimiento = $oCajasMovimientos->GetByIdCajaDetalleIdEntidad($IdCajaDetalle, TiposMovimientosCaja::CuentaCorrienteUsado, $IdCuentaGestoria))
		{
		...
		if ($oCajaMovimiento = $oCajasMovimientos->GetByIdCajaDetalleIdEntidad(1, TiposMovimientosCaja::CuentaCorrienteUsado, $IdCuentaGestoria))
		{
			$oCajasMovimientos->Delete($oCajaMovimiento->IdCajaMovimiento);
	...
	{
		$oCajasMovimientos = new CajasMovimientos();
		if ($oCajaMovimiento = $oCajasMovimientos->GetByIdEntidad(TiposMovimientosCaja::RendicionUsado, $IdCuentaGestoria))
		{
			$oCajasMovimientos->Delete($oCajaMovimiento->IdCajaMovimiento);
	...
	public function ExportXls(array $filter = NULL, $fullpermiso = false)
	{
		...
		{	
			...
			(
				...
				trim($oMinuta->Anticipo - $oCuentaGestoria->TotalFinal)
			);
