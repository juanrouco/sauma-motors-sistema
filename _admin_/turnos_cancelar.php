...
if ($Submit)
{
	...
	{		
			...
			$oTurnoComentario->IdTipoRechazo		= $IdTipoRechazo;
			$oTurnoComentario->IdUsuario 		= Session::GetCurrentUser()->IdUsuario;

			$oTurnosComentarios->Create($oTurnoComentario);
			
			$oTurno->IdEstadoOrden = EstadoOrden::Rechazado;
			//$oTurno->FechaInicio = date('d/m/Y H:i:s');
			$oTurnos->Update($oTurno);
			
			header("Location: turnos_detail.php" . $strParams . '&IdTurno=' . $oTurno->IdTurno);
			exit();
    	...
    	<td>
			<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
      			<tr>
        			...
        			<td height="40"><span class="tituloPagina">Administraci&oacute;n de Turnos - Rechazo de turno</span></td>
      			</tr>
    		</table>
		</td>
  	</tr>
					...
					<table width="50%"  border="0" align="center" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
						<tr>
							<td height="30">
								...
								<input type="button" name="btnCancelar" class="botonBasico" id="btnCancelar" onclick="javascript: window.location.href = 'turnos_detail.php<?=$strParams?>&IdTurno=<?= $oTurno->IdTurno ?>';" value="Cancelar" />
								</div>
							</td>
						</tr>
					</table>
