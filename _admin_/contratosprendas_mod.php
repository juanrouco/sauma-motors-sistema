...
$MontoSolicitado		= intval($_REQUEST['MontoSolicitado']);
$GastoOtorgamiento		= intval($_REQUEST['GastoOtorgamiento']);
$CostoOtorgamiento		= intval($_REQUEST['CostoOtorgamiento']);
$Comision				= strval($_REQUEST['Comision']);
$MontoAcreditado		= intval($_REQUEST['MontoAcreditado']);
...
$MontoOtorgado			= intval($_REQUEST['MontoOtorgado']);
...
if (!$oContratoPrenda = $oContratosPrendas->GetById($IdContratoPrenda))
{
	header('Location: contratosprendas.php');
	exit;
}
...
if ($Submit)
{
	...
	if ($err == 0)
	{
		...
		$oContratosPrendas->Update($oContratoPrenda);
	}
...
else
{
	...
	$CarpetaCompleta	= $oContratoPrenda->CarpetaCompleta;
	$PrePrenda			= $oContratoPrenda->PrePrenda;
	$PrendaInscripta	= $oContratoPrenda->PrendaInscripta;
	$FechaEnvioCarpeta	= CambiarFecha($oContratoPrenda->FechaEnvioCarpeta);
	$FechaAprobado		= CambiarFecha($oContratoPrenda->FechaAprobado);
	$FechaRechazado		= CambiarFecha($oContratoPrenda->FechaRechazado);
	$FechaObservacion	= CambiarFecha($oContratoPrenda->FechaObservacion);
	$Observacion		= $oContratoPrenda->Observacion;
	$IdEstado			= $oContratoPrenda->IdEstado;
	$FechaGestoria		= CambiarFecha($oContratoPrenda->FechaGestoria);
	$FechaEnvioPrenda	= CambiarFecha($oContratoPrenda->FechaEnvioPrenda);
}
...
{
	Get('IdMinuta').value 		= '';
	var oMinuta = GetMinuta(IdMinuta);
	...
	var oUnidad = GetUnidad(oMinuta.IdUnidad);
	...
	var oGestoria = GetCuentaGestoria(oMinuta.IdMinuta);
	
	Get('IdMinuta').value 		= oMinuta.IdMinuta;
	Get('NumeroCarpeta').value = oMinuta.IdMinuta;
	...
	var Facturad = 'NO';
	var Gestoria = 'NO';
	if (oUnidad.Certificado == 'true')
		Facturad = 'SI';
	if (oGestoria)
		Gestoria = 'SI';
	
	Get('Cliente').innerHTML+= '<br /><br /><strong>Facturado:</strong> ' + Facturad;
	Get('Cliente').innerHTML+= '<br /><br /><strong>Gestoria:</strong> ' + Gestoria;
}
    	...
    	<td>
			<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
      			<tr>
        			...
        			<td height="40"><span class="tituloPagina">Administraci&oacute;n de Contratos de Prenda - Modificar Contrato</span></td>
      			</tr>
    		</table>
		</td>
  	</tr>
						...
						<tr>
							<td height="30">
								<div align="center">
									...
									<input type="button" name="btnCancelar" class="botonBasico" id="btnCancelar" onclick="javascript: window.location.href = 'contratosprendas.php<?=$strParams?>';" value="Cancelar" />
								</div>
							</td>
						</tr>
					</table>
