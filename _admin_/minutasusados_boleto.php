...
if (!$oUsado = $oUsados->GetById($oMinutaUsado->IdUsado))
	exit();
...
if (!$oCliente = $oClientes->GetById($oMinutaUsado->IdCliente))
	exit();
...
$oUsado->IdEstado = EstadoUnidad::Vendido;
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center">
                    ...
                    <tr>
                        <td>
                            <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                <tr>
                                	<td width="100%" height="25"><?= utf8_encode('Conste por la presente que el señor') ?> VICTOR H. TOLOSA S.A. <?= utf8_encode('domiciliado en') ?> ACCESO NORTE KM 53,000 CUIT 30-67925285-9 <?= utf8_encode('vende y transfiere al señor') ?> <?=utf8_encode($oCliente->RazonSocial)?> <?= utf8_encode('domiciliado en') ?> <?=utf8_encode($oCliente->GetDomicilio())?> DNI <?=$oCliente->DocumentoNumero?> <?= utf8_encode('lo siguiente UN VEHICULO USADO') ?> Marca <?= utf8_encode($oMarca->Nombre) ?> Modelo <?= utf8_encode($oUsado->Modelo) ?> <?= utf8_encode('Año') ?> <?= $oUsado->ModeloAnio ?> Dominio <?= utf8_encode($oUsado->Dominio) ?> <?= utf8_encode('Motor Nº') ?> <?= utf8_encode($oUsado->NumeroMotor) ?> <?= utf8_encode('Chasis Nº') ?> <?= utf8_encode($oUsado->NumeroChasis) ?> <?= utf8_encode('en el estado en el que se encuentra tomando en la fecha el comprador posesión del mismo de conformidad.') ?></td>
                                </tr>
								<tr>
									<td width="100%" height="25"><?= utf8_encode('El precio de venta se establece en $') ?> <?= number_format($oMinutaUsado->PrecioVenta, 2, ',', '') ?> (<?= $oNumber->ValorEnLetras($oMinutaUsado->PrecioVenta, "pesos") ?>)</td>
								</tr>
								<tr>
									<td width="100%" height="25"><?= utf8_encode('Pagaderos en la siguiente forma: $') ?><?= number_format($oMinutaUsado->PrecioVenta, 2, ',', '') ?> (<?= $oNumber->ValorEnLetras($oMinutaUsado->PrecioVenta, "pesos") ?>)<?= utf8_encode(' que el comprador abona en el acto sirviendo el presente de suficiente recibo.') ?></td>
								</tr>
								<tr>
					...
					<tr>
                        <td>
                            <table width="100%" border="0" cellpadding="0" cellspacing="0">
                                <tr>
                                	<td width="100%">En <?= utf8_encode('OLIVOS') ?>, a los <?= date("d") ?> <?= utf8_encode('días del mes de') ?> <?= ObtenerMes(date("m")) ?> de <?= date("Y") ?></td>
                                </tr>
                            </table>
                        </td>
                    </tr>
