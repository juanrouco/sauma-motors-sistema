...
if ($Submit)
{	
	if ($oFacturaP = $oFacturasPostVentas->GetByCompra($oCompra))
		$err |= 2;
	...
	//print_r($TotalIndicado);exit;
	$totaldesc = ($oCompra->Total * ( 100 - $Descuento)/100) * (( 100 + $Interes) / 100);
	// print_r($totaldesc - $TotalIndicado);
	//print_R(TipoPago::GetIndexPV(99));exit;
	if (abs($totaldesc - $TotalIndicado) > 1)
	{
		$err |= 1;
	}
	...
	if ($err == 0)
	{
		...
		$oComprobante->Prefijo = str_pad(ConfiguracionFactura::PuntoVentaPV, 4, "0", STR_PAD_LEFT);
		$oComprobante->Numero = '00000000';
		$oComprobante->IdEstado = ComprobanteEstados::Libre;
		
		$oComprobante = $oComprobantes->Create($oComprobante);
		
		$oFacturaPostVenta = $oCompraFactura->GenerarFactura($oCompra, $oCliente, $oComprobante, $IdPlanCuota, $Descuento, $Comentarios, $Interes);
...
function ActualizarFacturacion()
{
	...
	var multiplicadorInteres = Interes / 100;
	...
	var TotalInteres = TotalOT2 * (1 + Interes / 100);
                                                        ...
                                                        <table border="0" align="center" cellpadding="0" cellspacing="0">                                                           
															...
															<tr>
																<td colspan="2">
																	<table border="0" align="left" cellpadding="0" cellspacing="0">
																		<tr>
																			<td><div id="margen" align="left">Inter&eacute;s:</div></td>
																		</tr>
																		<tr>
																			<td>
																				<div align="left">
																					<input type="text" name="Interes" id="Interes" class="camporFormularioChico" value="<?= $Interes ?>" />%
																				</div>
																			</td>
																		</tr>
																	</table>
																				...
																				<div align="left">
																					...
																					?>
																						<tr>
																							...
																							<td>
																								<div id="margen">
																									<input type="text" class="camporFormularioChico" <?= in_array($oTipoPago['IdTipoPago'], $arrIdTipoPago) ? '' : 'style="display: none"' ?> name="Importe[]" id="Importe_<?= $oTipoPago['IdTipoPago'] ?>" value="<?= $oTipoPago['IdTipoPago'] == 1 ?  $oCompra->Total : 0 ?>" />
																								</div>
																							</td>
																						</tr>
																					<?php
																					...
																					?>
																						<tr>
																							...
																							<td>
																								<div id="margen">
																									<input type="text" class="camporFormularioChico" <?= in_array(99, $arrIdTipoPago) ? '' : 'style="display: none"' ?> name="Importe[]" id="Importe_99" value="<?= $oCompra->Total ?>" />
																								</div>
																							</td>
																						</tr>
																					</table>
                                                            ...
                                                            <tr>
                                                                <td height="20">
																	...
																	<?php if ($err & 2) { ?><li style="color:#FF0000;">La venta ya ha sido facturada.</li><?php } ?>
																</td>
                                                            </tr>
															<tr>
																<td colspan="2">
