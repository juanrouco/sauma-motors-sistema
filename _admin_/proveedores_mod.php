...
if (!Session::CheckPerm(PERM_PROVE_UPDATE))
	Session::NoPerm();
...
$strParams.= '&FilterCuit='			. $_REQUEST['FilterCuit'];
...
/* validaciones... */
	...
	{
		if ($oProveedorAux->IdProveedor != $oProveedor->IdProveedor)
			$err |= 8;
	}
	...
	if ($err == 0)
	{
		...
		$oProveedor = $Proveedores->Update($oProveedor);
...
{		
		$Empresa				=	$oProveedor->Empresa;
		$IdRubro				=	$oProveedor->IdRubro;
		$TelefonoCodigoArea		=	$oProveedor->TelefonoCodigoArea;
		$Telefono				=	$oProveedor->Telefono;
		$TelefonoCodigoArea2	=	$oProveedor->TelefonoCodigoArea2;
		$Telefono2				=	$oProveedor->Telefono2;		
		$FaxCodigoArea			=	$oProveedor->FaxCodigoArea;
		$Fax					=	$oProveedor->Fax;
		$Email					=	$oProveedor->Email;
		$Web					=	$oProveedor->Web;
		$DomicilioCalle			=	$oProveedor->DomicilioCalle;
		$DomicilioNumero		=	$oProveedor->DomicilioNumero;
		$DomicilioPiso			=	$oProveedor->DomicilioPiso;
		$DomicilioDpto			=	$oProveedor->DomicilioDpto;
		$IdPais					=	$oProveedor->IdPais;
		$IdProvincia			=	$oProveedor->IdProvincia;
		$IdPartido				=	$oProveedor->IdPartido;
		$IdLocalidad			=	$oProveedor->IdLocalidad;
		$CodigoPostal			=	$oProveedor->CodigoPostal;
		$Observaciones			=	$oProveedor->Observaciones;
...
function FilterRubro(IdRubro, Nombre)
{
	...
	$j('#Rubro').val(oRubro.Nombre);
	$j('#IdRubro').val(oRubro.IdRubro);
}
    	...
    	<td>
			<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
      			<tr>
        			...
        			<td height="40"><span class="tituloPagina">Modificar Proveedor </span></td>
   			  </tr>
    		</table>
		</td>
  	</tr>
                           ...
                           <?php } ?>
                            ...
                            <?php } ?>
								...
								<tr>
									...
									<td>
										<div align="left">
											<input type="text" name="Partido" id="Partido" onkeyup="javascript: StrToUpper(this.id);ValidatePartido();" class="camporFormularioSuggest" maxlength="128" value="<?=$Partido?>" autocomplete="off" disabled="true" >
