...
if ($Submit)
{
	switch ($Action)
	{
		case 'Add':
			...
			$oOrdenesTrabajoTareasArticulos->Begin();
			try
			{
				foreach ($Ids as $Id)
				{
					...
					$update = true;
					if (!$oOrdenTrabajoTareaArticulo = $oOrdenesTrabajoTareasArticulos->GetById($IdOrdenTrabajoTarea, $Id))
					{
					...
					$oOrdenTrabajoTareaArticulo->Cantidad 				= $cantidad;
					$oIva = $oIvas->GetById($oArticulo->IdIva);
					$oOrdenTrabajoTareaArticulo->PrecioTotal = $oArticulo->PrecioLista * (1 + $oIva->Alicuota) * $cantidad;
					if (!$update)
						$oOrdenesTrabajoTareasArticulos->Create($oOrdenTrabajoTareaArticulo);
					else
						$oOrdenesTrabajoTareasArticulos->Update($oOrdenTrabajoTareaArticulo);
				}
				$oOrdenesTrabajoTareasArticulos->Commit();
			}
			catch (Exception $ex)
			{
				$oOrdenesTrabajoTareasArticulos->Rollback();
			}
		
			break;
			
		case 'Delete':
			$oOrdenesTrabajoTareasArticulos->Delete($oOrdenTrabajoTarea->IdOrdenTrabajoTarea, $Id);
			break;
...
$arrIvas 			= $oIvas->GetAll();
$arrArticulos 		= $oOrdenesTrabajoTareasArticulos->GetAllByOrdenTrabajoTarea($oOrdenTrabajoTarea);
						...
						<tr>							
							<td>
								<table width="100%" border="0" cellpadding="0" cellspacing="0">
									<tr>
										<td><strong>Relaci&oacute;n de repuestos para la tarea <?= $oOrdenTrabajoTarea->Titulo ?> de la orden N&deg; <?= $oOrdenTrabajo->IdOrdenTrabajo ?></strong></td>
									</tr>
								</table>
							</td>
						</tr>
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
						<tr>
							<td height="30">
								<div align="right">
									<label>
										<input name="button" type="button" class="botonBasico" id="button" onclick="javascript: window.location.href = 'ordenestrabajotareas.php<?=$strParams?>';" value="Volver a tareas de trabajo" />
									</label>
								</div>
							</td>
