	...
	switch ($Action)
	{
			...
			try
			{
				foreach ($Ids as $Id)
				{
					...
					if (!$oOrdenTrabajoTarea = $oOrdenesTrabajoTareas->GetById($IdOrdenTrabajo, $Id))
					{
						...
						$oOrdenTrabajoTarea->Tarea	 			= $oTareaTrabajo->Titulo;
						...
						$oOrdenTrabajoTarea->IdCategoria		= intval($_REQUEST['IdCategoria_' . $Id]);
						$oOrdenTrabajoTarea->IdVendedor			= intval($_REQUEST['IdVendedor_' . $Id]);
		...
		case 'AddNueva':
			...
			try
			{
				...
				$oOrdenTrabajoTarea->Tarea	 			= $_REQUEST['NuevaTarea'];
				$oOrdenTrabajoTarea->Descripcion 		= $_REQUEST['NuevaDescripcion'];
				$oOrdenTrabajoTarea->IdTipoVenta		= intval($_REQUEST['NuevoIdTipoVenta']);
				$oOrdenTrabajoTarea->IdCategoria		= intval($_REQUEST['NuevoIdCategoria']);
				$oOrdenTrabajoTarea->IdCodigoTrabajo	= intval($_REQUEST['NuevaIdCodigoTrabajo']);
				$oOrdenTrabajoTarea->IdVendedor			= intval($_REQUEST['NuevoIdVendedor']);
		...
		case 'Delete':
			...
			try
			{
				...
				if ($oOrdenTrabajoTarea->CantidadRepuestosAsignados() > 0)
				{
					$AlertaRepuestos = true;
				}
				else
				{
...
function AgregarTarea(idTarea) {
	if (idTarea != '' && idTarea != null && idTarea != undefined) {
		if ($j('#id_' + idTarea).length == 0) {
			...
			row += "	</td>";
			row += "	<td height=\"25\">";
			row += "		<select id=\"IdCategoria_" + oTareaTrabajo.IdTareaTrabajo + "\" name=\"IdCategoria_" + oTareaTrabajo.IdTareaTrabajo + "\">";
			<?php
				foreach (Categorias::GetAll() as $oCategoria)
				{
			?>
			row += "			<option value=\"<?= $oCategoria['IdCategoria'] ?>\"><?= $oCategoria['Nombre'] ?></option>";
			<?php
				}
			?>
			row += "		</select>";
			row += "	</td>";
			row += "	<td height=\"25\">";
			row += "		<select id=\"IdVendedor_" + oTareaTrabajo.IdTareaTrabajo + "\" name=\"IdVendedor_" + oTareaTrabajo.IdTareaTrabajo + "\">";
			row += "			<option value=\"\">Indistinto</option>";
			<?php
				foreach ($arrUsuarios as $oUsuario)
				{
			?>
			row += "			<option value=\"<?= $oUsuario->IdUsuario ?>\"><?= $oUsuario->Nombre . ' ' . $oUsuario->Apellido ?></option>";
			<?php
				}
			?>
			row += "		</select>";
...
$j(document).ready(function() {
	...
	<?php
	if ($AlertaRepuestos)
		echo 'alert("No es posible eliminar la tarea.\nTiene repuestos asignados");';
	?>
...
<tr>
				<td>
					<div align="center">
						<div id="ShownAddRelacion" style="width:33%; border: none; float: left; height: 20px; padding-top: 5px" class="bordeGrisFondo"><font>[+]<a href="#bottom" class = "linkMenu" onclick="javascript: ShowAddRelacion();"><b> A&ntilde;adir Tareas Predeterminadas</b></a></font></div>
						<div id="ShownAddNueva" style="width:34%; border: none; float: left; height: 20px; padding-top: 5px" class="bordeGrisFondo"><font>[+]<a href="#bottom" class = "linkMenu" onclick="javascript: ShowAddNueva();"><b> A&ntilde;adir Nueva Tarea</b></a></font></div>
						<div id="ShownAddTerceros" style="width:33%; border: none; float: left; height: 20px; padding-top: 5px" class="bordeGrisFondo">
							<font>
								<a href="ordenestrabajotareas_terceros_add.php?IdOrdenTrabajo=<?= $IdOrdenTrabajo ?>" class="linkMenu"><b> A&ntilde;adir Tarea Terceros</b></a>
							</font>
						</div>
																...
																<td colspan="6">
																	<table id="tareas" class="bordeGris" border="0" align="center" cellpadding="0" cellspacing="0">
																		<tr class="bordeGrisFondo">
																			...
																			<td width="100" height="25" class="bordeGrisTitulo"><strong>Categor&iacute;a</strong></td>
																			<td width="100" height="25" class="bordeGrisTitulo"><strong>Vendedor</strong></td>
												...
												<tr>
													<td>
														<table width="70%" border="0" cellpadding="0" cellspacing="0" align="center">
															<tr>
																<td width="40%" class="tituloMenu"><div align="right">Novedad del Cliente:&nbsp;&nbsp;</div></td>
																<td width="60%">
															...
															<tr>
																<td class="tituloMenu"><div align="right">Codigo de Trabajo:</div></td>
                                                                <td>
																	<div align="left">
																		...
																		<a id="buscar-codigos" href="#"><img src="images/iconos/lupa.jpg" alt="Buscar" title="Buscar" class="buscar2" style="margin-bottom: -6px" /></a>
															...
															<tr>
																...
																<td class="tituloMenu"><div align="right">Diagnostico:&nbsp;&nbsp;</div></td>
																<td>
															...
															<tr>
																<td class="tituloMenu"><div align="right">Categor&iacute;a:&nbsp;&nbsp;</div></td>
																<td>
																	<select id="NuevoIdCategoria" name="NuevoIdCategoria" style="width: 250px">
																	<?php
																	foreach (Categorias::GetAll() as $oCategoria)
																	{
																	?>
															...
															<tr>
																...
																<td>
																	<select id="NuevoIdVendedor" name="NuevoIdVendedor" style="width: 250px">
																		<option value="">Indistinto</option>
																	<?php
																	foreach ($arrUsuarios as $oUsuario)
																	{
																	?>
																		<option value="<?= $oUsuario->IdUsuario ?>"><?= $oUsuario->Nombre . ' ' . $oUsuario->Apellido ?></option>
																	<?php
																	}
																	?>
																	</select>
															...
															<tr>
																<td width="40%" class="tituloMenu"><div align="right">Tarea Realizada:&nbsp;&nbsp;</div></td>
																<td width="60%">
																	<div align="left"><input type="text" name="NuevaTarea2" id="NuevaTarea2" value="<?= $NuevaTarea2 ?>" onkeyup="javascript: StrToUpper(this.id);" style="width:250px" /></div>
																</td>
															</tr>
															<tr>
																<td colspan="2">&nbsp;</td>
				...
				<td>
					<table width="100%" align="center" cellpadding="0" cellspacing="0" class="bordeGris">
						<tr class="bordeGrisFondo">							
							...
							<td width="150" height="25"><div id="margen"><strong>Categor&iacute;a</strong></div></td>
						...
						{ 
							...
							$oCategoria = Categorias::GetById($oRelacion->IdCategoria);
						?>
						<tr onMouseOver="bgColor='#f3f3f3'" onMouseOut="bgColor=''">
							<td width="350"><div id="margen"><?=$oRelacion->Titulo?></div></td>
							<td width="350"><div id="margen"><?= $oTipoVenta['Nombre'] ?></div></td>
							<td width="350"><div id="margen"><?= $oCategoria['Nombre'] ?></div></td>
							<td width="100" height="25"><div id="margen">$<?=$oRelacion->ImporteRepuestos()?></div></td>	
							<td><div id="margen">$<?=$oRelacion->Importe ?></div></td>
							<td>
								<div align="center">
								<?php
								if ($oRelacion->Terceros)
								{
								?>
									<a href="ordenestrabajotareas_terceros_mod.php<?=$strParams?>&IdOrdenTrabajoTarea=<?= $oRelacion->IdOrdenTrabajoTarea ?>&IdOrdenTrabajo=<?= $oRelacion->IdOrdenTrabajo ?>">
										<img src="images/iconos/mod.gif" alt="Modificar" border="0" /></a> - 
									<a href="ordenestrabajotareas_terceros_del.php<?=$strParams?>&IdOrdenTrabajoTarea=<?= $oRelacion->IdOrdenTrabajoTarea ?>&IdOrdenTrabajo=<?= $oRelacion->IdOrdenTrabajo ?>">
								...
								{
								?>
									<a href="ordenestrabajotareas_mod.php<?=$strParams?>&IdOrdenTrabajoTarea=<?= $oRelacion->IdOrdenTrabajoTarea ?>">
										<img src="images/iconos/mod.gif" alt="Modificar" border="0" /></a> - 
									<a href="ordenestrabajotareasarticulos.php<?=$strParams?>&IdOrdenTrabajoTarea=<?= $oRelacion->IdOrdenTrabajoTarea ?>">
										<img src="images/iconos/relacion.png" alt="Repuestos Relacionados" border="0" /></a> - 
									<a href="#bottom" onClick="javascript: Delete(<?=$oRelacion->IdOrdenTrabajoTarea?>)">
										<img src="images/iconos/del.gif" alt="Eliminar" border="0" />
									</a>
								<?php
								}
					...
					<table width="100%" border="0" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
						<tr>
							<td height="30">
								<div align="right">
									<label>
										<input name="button" type="button" class="botonBasico" id="button" onclick="javascript: window.location.href = 'ordenestrabajo_detail.php<?=$strParams?>';" value="Finalizar" />
									</label>
								</div>
							</td>
