...
{	
	...
	$totaldesc = ($oCompra->Total - $oCompra->PercepcionIIBB) * (100-$Descuento)/100;
	//print_r($totaldesc);Exit;
	...
	if ($err == 0)
	{
		$oCompra->Total = $oCompra->Total - $oCompra->PercepcionIIBB;
		$oCompra->PercepcionIIBB = 0;
		$oCliente = $oClientes->GetById($IdCliente);
		...
		$oComprobante->PercepcionIIBB = 0;//$oFacturaPostVenta->PercepcionIIBB;
		...
		$oCompra->PercepcionIIBB = 0;//$oFacturaPostVenta->PercepcionIIBB;
...
var TotalOT = <?= $oCompra->Total - $oCompra->PercepcionIIBB ?>;
var IvaOT = <?= $oCompra->Iva21 ?>;
var PercIIBBOT = 0;//<?= $oCompra->PercepcionIIBB ?>;
...
function ActualizarFacturacion()
{
	...
	if (IdPlanCuota == null || IdPlanCuota == '' || IdPlanCuota == '0') {
		if ($j('#IdTipoPago_14').length > 0 &&  $j('#IdTipoPago_14')[0].checked) {
			IdPlanCuota = '60';
		} else {
    	...
    	<td>
			<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
      			<tr>
        			...
        			<td height="40"><span class="tituloPagina">Administraci&oacute;n de Ventas de Mostrador - Agregar Recibo</span></td>
      			</tr>
    		</table>
		</td>
  	</tr>
																				...
																				<div align="left">
																					...
																					?>
																						<tr>
																							...
																							<td>
																								<div id="margen">
																									<input type="text" class="camporFormularioChico" <?= in_array($oTipoPago['IdTipoPago'], $arrIdTipoPago) ? '' : 'style="display: none"' ?> name="Importe[]" id="Importe_<?= $oTipoPago['IdTipoPago'] ?>" value="<?= $oTipoPago['IdTipoPago'] == 1 ?  $oCompra->Total - $oCompra->PercepcionIIBB : 0 ?>" />
																								</div>
																							</td>
																						</tr>
																					<?php
																					...
																					?>
																						<tr>
																							...
																							<td>
																								<div id="margen">
																									<input type="text" class="camporFormularioChico" <?= in_array(99, $arrIdTipoPago) ? '' : 'style="display: none"' ?> name="Importe[]" id="Importe_99" value="<?= $oCompra->Total- $oCompra->PercepcionIIBB ?>" />
																								</div>
																							</td>
																						</tr>
																					</table>
