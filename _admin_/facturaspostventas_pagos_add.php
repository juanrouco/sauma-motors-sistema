...
$oFactura = $oFacturasPostVentas->GetById($IdFacturaPostVenta);
...
if ($Submit)
{
	...
	if (($Importe == '') || (floatval($Importe) == 0))
		$err |= 4;
	if ($IdTipoPago == TipoPago::DepositoEfectivo || $IdTipoPago == TipoPago::DepositoCheque || $IdTipoPago == TipoPago::Transferencia	)
	{
		if ($BancoDestino == '')
			$err |= 8;
	}
	...
	else
	{
		if ($NumeroRecibo != '')
		{
			$oPagoAux = $oPagos->GetByNumeroRecibo($NumeroRecibo);
			if ($oPagoAux)
				$err |= 8;
		}
	}*/
    	...
    	<td>
			<table width="100%"  border="0" cellpadding="0" cellspacing="0" class="bordeGrisFondo">
      			<tr>
        			...
        			<td height="40"><span class="tituloPagina">Administraci&oacute;n de Facturas de Posventas - Agregar Pago</span></td>
      			</tr>
    		</table>
		</td>
  	</tr>
					  	...
					  	<tr>
							<td class="bordeGris">
								<table border="0" align="center" cellpadding="0" cellspacing="0">
                                	...
                                	<tr>
										<td height="20">&nbsp;</td>
										<td height="20" align="left"><?php if ($err & 88) { ?><li style="color:#FF0000;">El nro de recibo ya fue ingresado.</li><?php } ?><?php if ($err & 16) { ?><li style="color:#FF0000;">Ingrese un nro de recibo.</li><?php } ?></td>
									</tr>
									<tr>
                                                            ...
                                                            <div align="left">
                                                              <select name="BancoDestino" id="BancoDestino" class="camporFormularioSimple">
																...
																<?php foreach ($arrCajasDeposito as $oCajaDetalle) { ?> 
																	<option value="<?=$oCajaDetalle->IdCajaDetalle?>" <?= $BancoDestino == $oCajaDetalle->IdCajaDetalle ? 'selected="selected"' : '' ?>><?=$oCajaDetalle->Nombre?></option>
																<?php } ?>
															</select>
                                                            </div>
                                                        </td>
						...
						<tr>
							<td height="30">
								<div align="center">
									...
									<input type="button" name="btnCancelar" class="botonBasico" id="btnCancelar" onclick="javascript: window.location.href = 'facturaspostventas_pagos.php<?=$strParams?>';" value="Cancelar" />
								</div>
							</td>
						</tr>
					</table>
